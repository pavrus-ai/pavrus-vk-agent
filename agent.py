...
def good_text(t):
    ...
    think = ["thinking process", "analyze the request", "constraints:", "**role", "source text:",
             "here's a thinking", "<think>", "additional text:"]
...
def score_text(t):
    """Баллы за качество: длина, эмодзи, абзацы, русская речь, ссылка."""
    s = 0
    n = len(t)
    if 350 <= n <= 700: s += 2
    if 450 <= n <= 650: s += 1
    ...
    if re.search(r"[а-яё]", t.lower()) and len(re.findall(r"[а-яё]", t.lower())) > 100: s += 2
    if "**" in t: s -= 2
    return s
...
# ---------- Этап 4: ДВА ИИ пишут пост, публикуем лучший ----------
def template_text():
    base = desc or body
...
if OR_KEY:
    for model in or_free_models()[:3]:
        try:
            ...
            a = a.replace("**", "").strip()
            if good_text(a):
                candidates.append((a, f"OpenRouter ({model})", b))
                break
        except Exception as e:
            log("Этап 4 (debug)", "⚠️", f"OpenRouter {model}: {str(e)[:100]}")
if GROQ_KEY:
    try:
        ...
        a = a.replace("**", "").strip()
        if good_text(a):
            candidates.append((a, "Groq", b))
    except Exception as e:
        log("Этап 4 (debug)", "⚠️", f"Groq: {str(e)[:100]}")

text, src, img_hint = None, "", ""
if candidates:
    candidates.sort(key=lambda c: score_text(c[0]), reverse=True)
    text, src, raw_img = candidates[0]
    img_hint = raw_img if good_img(raw_img) else ""
    if len(candidates) > 1:
        log("Этап 4 (сравнение)", "ℹ️",
            " | ".join(f"{c[1]}: {score_text(c[0])} балл." for c in candidates) + f" → выбран: {src}")
if not text:
